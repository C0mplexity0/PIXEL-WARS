...
var myGameArea = {
  ...
  start : function() {
    this.canvas.width = zoom*17;
    this.canvas.height = zoom*17;
...
function player(x, y) {
  ...
  this.update = function(){
    ...
    ctx.fillRect((8*zoom)+(0.1*zoom), (8*zoom)+(0.1*zoom), (0.8*zoom), (0.8*zoom));
    ctx.font = "16px pixel";
    ctx.textAlign = "center";
    ctx.fillText(username, (8*zoom)+(zoom*0.5), (8*zoom)-(zoom*0.2));
  }
...
function updateGameArea() {
  if (window.innerWidth > 32*17 && window.innerHeight > 32*17) {
  zoom = 32;
  } else if (window.innerWidth > 24*17 && window.innerHeight > 24*17) {
    zoom = 24;
  } else if (window.innerWidth > 16*17 && window.innerHeight > 16*17) {
    zoom = 16;
  } else {
    zoom = 8;
  }
  myGameArea.canvas.width = zoom*17;
  myGameArea.canvas.height = zoom*17;
  fps++;
  ...
  for (var y=0;y<17;y++) {
    for (var x=0;x<17;x++) {
      var block = getBlock(playerPiece.x+(x-8), playerPiece.y+(y-8));
      if (block > 0) {
        ctx = myGameArea.context;
        if (blocks[block-1] == "img") {
          ctx.drawImage(blockTextures[block-1] , x*zoom, y*zoom, zoom, zoom);
        } else {
      ...
      for (var i=0;i<playerList.length;i++) {
        if (playerList[i][1] != null) {
          if (playerList[i][0] != username) {
            if (playerList[i][1][0] == playerPiece.x+(x-8) && playerList[i][1][1] == playerPiece.y+(y-8)) {
              ctx = myGameArea.context;
